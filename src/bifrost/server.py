...
async def page(request):
    return web.Response(
        text=SHELL, content_type="text/html", headers={"Cache-Control": "no-store"}
    )
...
async def signal(request):
    ...
    if role == "agent":
        if state["agent"] and not state["agent"].closed:
            ...
            return ws
        state["agent"] = ws
        log.info("agent connected room=%s", room)
    else:
        ...
        async for msg in ws:
            if msg.type == WSMsgType.TEXT:
                ...
                target = (
                    state["agent"]
                    if role == "client"
                    else next((x for x in state["clients"] if not x.closed), None)
                )
...
def main():
    ...
    web.run_app(
        app, host=cfg["server"]["bind"], port=cfg["server"]["port"], ssl_context=ctx
    )
