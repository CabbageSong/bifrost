...
浏览器 iframe 中的超链接、表单提交、程序化页面跳转、`fetch`、`XMLHttpRequest` 和 `sendBeacon` 会被桥接到同一条 DataChannel。请求方法、请求头和请求体会传给内网 client，因此 `GET`、`POST`、`PUT`、`PATCH`、`DELETE`、`HEAD` 及其他 aiohttp 接受的方法都可以转发；二进制请求体和响应体使用 Base64 保留原始字节。client 会为同一 WebRTC peer 复用 HTTP session，因此连接池和 Cookie 可以跨请求保留。
